...
static void dbg_log_event(struct urb *urb, char *event, unsigned int extra)
{
	unsigned long flags;
	int ep_addr;
	...
	if ((ep_addr & 0x0f) == 0x0) {
		...
		if (!str_to_event(event)) {
			...
			scnprintf(dbg_hsic_ctrl.buf[dbg_hsic_ctrl.idx],
				DBG_MSG_LEN,
		"%s: [%s : %pK]:[%s] %02x %02x %04x %04x %04x  %u %d %s",
...
static int __maybe_unused ulpi_read(struct msm_hsic_hcd *mehci, u32 reg)
{
	...
	if (cnt >= ULPI_IO_TIMEOUT_USEC) {
		dev_err(mehci->dev, "%s: timeout ULPI_VIEWPORT: %08x\n",
				__func__, readl_relaxed(USB_ULPI_VIEWPORT));
		...
		udelay(130);
		dev_err(mehci->dev, "%s: FRINDEX: %08x\n", __func__,
				readl_relaxed(USB_FRINDEX));
		return -ETIMEDOUT;
	}
...
static void msm_hsic_clk_reset(struct msm_hsic_hcd *mehci)
{
	...
	if (!IS_ERR(mehci->alt_core_clk)) {
		ret = reset_control_assert(mehci->core_reset);
		if (ret) {
		...
		ret = reset_control_deassert(mehci->core_reset);
		if (ret)
...
static int msm_hsic_start(struct msm_hsic_hcd *mehci)
{
	...
	if (mehci->tlmm_regs) {
		...
		if (ret) {
			pr_err(
	"%s: Unable to program length of connect signaling\n", __func__);
		}
...
#define MSM_USB_ASYNC_BRIDGE_BYPASS BIT(31)
	...
	if (pdata->dis_internal_clk_gating) {
		...
		temp &= ~GENCONFIG_2_SYS_CLK_HOST_DEV_GATE_EN;
...
static struct hc_driver msm_hsic_driver = {
	...
	.product_desc		= "Qualcomm Technologies, Inc. EHCI HSIC",
...
static int msm_hsic_init_clocks(struct msm_hsic_hcd *mehci, u32 init)
{
	...
	mehci->core_reset = devm_reset_control_get(mehci->dev, "core_reset");
	if (IS_ERR(mehci->core_reset)) {
		ret = PTR_ERR(mehci->core_reset);
		mehci->core_reset = NULL;
		if (ret != -EPROBE_DEFER)
			dev_err(mehci->dev, "failed to get core_reset\n");
		return ret;
	}
	...
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (!strcmp(buf, "enable")) {
		...
		if (mehci->bus_perf_client) {
			...
			if (ret)
				dev_err(mehci->dev,
		"%s: Failed to devote for bus bw %d\n", __func__, ret);
		}
	}
...
static int ehci_hsic_msm_probe(struct platform_device *pdev)
{
	...
	if (pdata && pdata->bus_scale_table) {
		...
		if (mehci->bus_perf_client) {
			...
			dev_err(&pdev->dev,
		"%s: Failed to register BUS scaling client!!\n", __func__);
		}
	}
