...
static void rfcomm_dlc_unlink(struct rfcomm_dlc *d)
{
	...
	BT_DBG("dlc %pK refcnt %d session %pK",
	       d, refcount_read(&d->refcnt), s);
			...
			u8 xon_char, u8 xoff_char, u16 param_mask)
{
	...
	BT_DBG("%pK cr %d dlci %d bit_r 0x%x data_b\
			 0x%x stop_b 0x%x parity 0x%x\
...
static struct rfcomm_session *rfcomm_process_rx(struct rfcomm_session *s)
{
	...
	BT_DBG("session %pK state %ld qlen %d",
